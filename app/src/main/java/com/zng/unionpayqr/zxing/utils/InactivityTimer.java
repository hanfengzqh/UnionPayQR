...
package com.zng.unionpayqr.zxing.utils;
...
public class InactivityTimer {
	...
	static final long INACTIVITY_DELAY_MS =  60 * 1000L;
	...
	public synchronized void onResume() {
		if (registered) {
			...
			Logger.i("liujie", "注册powerStatusReceiver");
	...
	class PowerStatusReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			Logger.i("liujie", "注册ACTION_BATTERY_CHANGED");
			if (Intent.ACTION_BATTERY_CHANGED.equals(intent.getAction())) {
				// 0 indicates that we're on battery
				boolean onBatteryNow = intent.getIntExtra(BatteryManager.EXTRA_PLUGGED, -1) <= 0;
				Logger.i("liujie", "onBatteryNow==="+onBatteryNow);
				if (onBatteryNow) {
	...
	class InactivityAsyncTask extends AsyncTask<Object, Object, Object> {
		...
		protected Object doInBackground(Object... objects) {
			try {
				Logger.i("liujie", "start---INACTIVITY_DELAY_MS");
				Thread.sleep(INACTIVITY_DELAY_MS);
				Logger.i("liujie", "Finishing---INACTIVITY_DELAY_MS");
